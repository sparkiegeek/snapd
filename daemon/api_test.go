...
func (s *postCreateUserSuite) SetUpTest(c *check.C) {
	...
	s.daemon(c)
	checkRootUserUcrednetGetUID = func(string) (uint32, error) {
		return 0, nil
	}
...
func mockCheckRootUserUcrednetGetUID(f func(string) (uint32, error)) func() {
	saved := checkRootUserUcrednetGetUID
	checkRootUserUcrednetGetUID = f
	return func() {
		checkRootUserUcrednetGetUID = saved
	}
}

// Install ucred uid mock to return known uid, returns func to restore original.
func mockUcredUID(uid uint32) func() {
	return mockCheckRootUserUcrednetGetUID(func(_ string) (uint32, error) {
		return uid, nil
	})
}
...
func (s *storeSuite) TestPut(c *check.C) {
	...
	st.Lock()
	stateAPI := storestate.API(st)
	after := storestate.Store(st)
	st.Unlock()
	c.Check(stateAPI, check.Equals, "https://example.com/")
...
func (s *storeSuite) TestPutNotRoot(c *check.C) {
	...
	c.Check(rsp.Status, check.Equals, 400)
	c.Check(rsp.Result, check.DeepEquals, &errorResult{
		Message: "cannot use store as non-root",
	})
}
...
func (s *storeSuite) TestDelete(c *check.C) {
	...
	c.Check(storestate.API(st), check.Not(check.Equals), "")
	st.Unlock()
